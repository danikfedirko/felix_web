...
class Home extends React.Component {
  ...
  {this.state.infoToggle ?
   this.state.markersData.map(marker => {
     ...
     return(
       <Card key={marker.id} title={marker.name} bordered={false} style={{ width: 300,position:'fixed',left:'20px', top:'120px',zIndex:10000 }}>
          <Carousel>
          {marker.photos.map((url,index) => {
            return(<img src={url} key={index} />)
            })
          }
          </Carousel>
          <div className='marker-meta'>
            <span>{marker.author}</span> <span style={{float:'right'}}>{marker.date}</span>
          </div>
